...
// Sequential solution
  ...
  private static long countIntStream(int range) {
    long count= 0;
    //to be filled out
    //IntStream.range(0,range).filter(x -> isPrime(x)).forEach(x -> System.out.println(x));
    count = IntStream.range(0,range).filter(x -> isPrime(x)).count();
    return count;
  }
  ...
  private static long countParallel(int range) {
    long count= 0;
    // to be filled out
    count = IntStream.range(0, range).parallel().filter(i -> isPrime(i)).count();
    return count;
  }
...
// parallelStream solution
  private static long countparallelStream(List<Integer> list) {
    long count= 0;
    // to be filled out
    count = IntStream.range(0, range).filter(i -> isPrime(i)).parallel().count();
    return count;
  }
